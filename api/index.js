...
async function evaluateLeadQualificationWithOpenAI(input) {
  if (!OPENAI_API_KEY) {
    const score = Number(input.statedBudget) >= 2e6 ? 92 : 75;
    return {
      score,
...
async function generateSetterHandoffScriptWithOpenAI(lead) {
  if (!OPENAI_API_KEY) {
    return {
