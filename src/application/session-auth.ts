...
export function signSession(userId: string, tenantId: string, secret = process.env.SESSION_SECRET): string {
  if (!secret) throw new Error('SESSION_SECRET is required');
  const now = Math.floor(Date.now() / 1000);
  const header = base64url(JSON.stringify({ alg: 'HS256', typ: 'JWT' }));
  const payload = base64url(JSON.stringify({ sub:userId, tenant_id:tenantId, iat:now, exp:now + Number(process.env.SESSION_TTL_SECONDS ?? 3600) }));
  const unsigned = `${header}.${payload}`;
  const signature = createHmac('sha256', secret).update(unsigned).digest('base64url');
  return `${unsigned}.${signature}`;
}

export function verifySession(token: string, secret = process.env.SESSION_SECRET): SessionClaims {
  if (!secret) throw new Error('SESSION_SECRET is required');
  ...
  if (!claims.sub || !claims.tenant_id || !claims.exp || claims.exp <= Math.floor(Date.now()/1000)) throw new Error('Session expired');
  return claims;
}
