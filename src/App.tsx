...
export default function App() {
  ...
  const runDemo = () => {
    setLeads((current) => current.map((lead) => lead.state === 'qualifying' ? { ...lead, state: 'qualified' } : lead));
  };
    ...
    <div className="min-h-screen bg-[#080b14] text-slate-100">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-slate-800 bg-[#0b0f1a] p-5 lg:block">
        ...
        <nav className="space-y-1">
          {nav.map(([label, Icon]) => <button key={label} onClick={() => setActive(label)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm ${active === label ? 'bg-indigo-500/15 text-indigo-300' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white'}`}><Icon size={17}/>{label}</button>)}
        </nav>
      ...
      <main className="lg:pl-64">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-800 bg-[#080b14]/90 px-5 py-4 backdrop-blur md:px-8">
          <div><div className="text-xs uppercase tracking-[0.18em] text-indigo-300">Revenue operating system</div><h1 className="mt-1 text-xl font-bold">{active}</h1></div>
          <button onClick={runDemo} className="flex items-center gap-2 rounded-xl bg-indigo-500 px-4 py-2 text-sm font-semibold hover:bg-indigo-400">Run qualification pass <ArrowRight size={16}/></button>
        </header>

        <section className="space-y-6 p-5 md:p-8">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Metric icon={<Inbox size={18}/>} label="Leads in engine" value={metrics.total}/>
            <Metric icon={<CheckCircle2 size={18}/>} label="Qualified" value={metrics.qualified}/>
            <Metric icon={<Clock3 size={18}/>} label="Avg first response" value={`${metrics.response}m`}/>
            <Metric icon={<CalendarIcon/>} label="Booked" value={metrics.booked}/>
          </div>

          <div className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
            <section className="card overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-800 p-5"><div><h2 className="font-semibold">Live lead pipeline</h2><p className="mt-1 text-xs muted">Every lead is scored before human routing.</p></div><span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-300">{leads.length} active</span></div>
              <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs text-slate-500"><tr><th className="px-5 py-3">Lead</th><th>Source</th><th>Score</th><th>State</th><th>Route</th></tr></thead><tbody>
