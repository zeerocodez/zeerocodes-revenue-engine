...
export interface RequestTransaction { client: PoolClient; finish: (commit: boolean) => Promise<void>; }
...
export class PostgresDatabase {
  ...
  async beginRequestTenant(tenantId:string):Promise<RequestTransaction>{
    ...
    let finished=false;
    return {client,finish:async(commit)=>{if(finished)return;finished=true;try{await client.query(commit?'COMMIT':'ROLLBACK');}finally{client.release();}}};
  }
