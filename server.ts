...
const leadAI=new LeadAIAgentOrchestrator(leadStore,new LeadResponseAgent(conversationService),new LeadQualificationAgent(qualificationOrchestrator));
...
app.get('/api/health',(_req,res)=>res.json({status:'ok',service:'zeerocodes-revenue-engine',mode:usePostgres?'postgres':'memory',workflow:'lead-to-revenue',rls:usePostgres,aiAgents:true}));
app.post('/api/auth/dev-session',(req,res)=>{if(process.env.NODE_ENV==='production')return res.status(404).json({error:'Not found'});const tenantId=String(req.body?.tenantId||process.env.DEV_TENANT_ID||'demo-tenant'),userId=String(req.body?.userId||process.env.DEV_USER_ID||'demo-user');return res.json({token:signSession(userId,tenantId),tenantId,userId});});

async function authMiddleware(req:RequestWithContext,res:express.Response,next:express.NextFunction){try{let context:AuthenticatedRequestContext;const bearer=req.header('authorization');if(bearer?.startsWith('Bearer '))context=await sessionIdentityResolver.resolveBearer(bearer.slice(7));else if(process.env.NODE_ENV!=='production'){const userId=req.header('x-user-id'),tenantId=req.header('x-tenant-id');if(!userId||!tenantId)return res.status(401).json({error:'Bearer session required'});context=await headerIdentityResolver.resolve({userId,tenantId});}else return res.status(401).json({error:'Bearer session required'});req.context=context;if(db){const transaction=await db.beginRequestTenant(context.tenantId);db.runRequestContext(context.tenantId,transaction.client,()=>{let settled=false;const finish=async()=>{if(settled)return;settled=true;await transaction.finish(res.statusCode<500);};res.on('finish',()=>{void finish();});next();});return;}next();}catch(error){return res.status(401).json({error:error instanceof Error?error.message:'Authentication failed'});}}
...
app.post('/api/leads/intake',async(req:RequestWithContext,res)=>{try{const c=contextOf(req);requireRole(c,'agent');const result=await revenueEngine.intake({...req.body,organizationId:c.tenantId});const configuration=await clientConfigurationService.get(c.tenantId);const agentResult=await leadAI.onLeadCreated(result.lead,configuration,req.body.channel||'web');if(followUpRepository)await planInitialFollowUps(followUpRepository,result.lead,result.decision);if(webhookDispatcher)await webhookDispatcher.enqueue({id:randomUUID(),organizationId:c.tenantId,type:'lead.created',occurredAt:new Date().toISOString(),payload:{lead:result.lead,decision:result.decision,ai:{response:agentResult.response,responseSent:agentResult.responseSent}}});return res.status(201).json({...result,ai:agentResult});}catch(e){return res.status(tenantError(e)).json({error:e instanceof Error?e.message:'Invalid lead intake'});}});
...
app.post('/api/leads/:id/messages',async(req:RequestWithContext,res)=>{try{const c=contextOf(req);requireRole(c,'agent');const channel=req.body.channel||'web';const inbound=await conversationService.sendMessage({...req.body,leadId:req.params.id,organizationId:c.tenantId,channel,actor:'lead'});const configuration=await clientConfigurationService.get(c.tenantId);const agentResult=await leadAI.onInboundMessage({leadId:req.params.id,organizationId:c.tenantId,text:inbound.message.body,configuration,channel});return res.json({inbound,ai:agentResult});}catch(e){const m=e instanceof Error?e.message:'Unable to process lead message';return res.status(m==='Tenant access denied'?403:tenantError(e)).json({error:m});}});
...
app.post('/api/leads/:id/redecide',async(req:RequestWithContext,res)=>{try{const c=contextOf(req);requireRole(c,'manager');return res.json(await revenueEngine.redecide(req.params.id,c.tenantId));}catch(e){const m=e instanceof Error?e.message:'Unable to redecide lead';return res.status(tenantError(e)).json({error:m});}});

app.post('/api/follow-ups/run',async(req:RequestWithContext,res)=>{try{const c=contextOf(req);requireRole(c,'manager');if(!followUpWorker)return res.status(503).json({error:'Follow-up worker requires PostgreSQL'});return res.json(await followUpWorker.runOnce(c.tenantId,Math.min(Number(req.body?.limit||25),100)));}catch(e){return res.status(tenantError(e)).json({error:e instanceof Error?e.message:'Unable to run follow-ups'});}});

app.post('/api/leads/:id/appointments',async(req:RequestWithContext,res)=>{try{const c=contextOf(req);requireRole(c,'agent');if(!workflow)return res.status(503).json({error:'Workflow persistence requires PostgreSQL'});const lead=await revenueEngine.getLeadForOrganization(req.params.id,c.tenantId);const idempotencyKey=req.header('idempotency-key')?.trim();if(idempotencyKey){const existing=await workflow.findAppointmentByIdempotency(c.tenantId,idempotencyKey);if(existing){if(existing.leadId!==lead.id)throw new Error('Idempotency key already belongs to another lead');return res.status(200).json({appointment:existing,lead});}}if(!['qualified','nurture'].includes(lead.state))throw new Error('Lead must be qualified before booking');const scheduledAt=new Date(req.body.scheduledAt);if(Number.isNaN(scheduledAt.getTime()))throw new Error('Invalid scheduledAt');const now=new Date().toISOString();const previousState=lead.state;const nextState=transitionLead(previousState,'booked');const appointment={id:randomUUID(),organizationId:c.tenantId,leadId:lead.id,scheduledAt:scheduledAt.toISOString(),status:'scheduled' as const,ownerUserId:c.userId,source:req.body.source,idempotencyKey,metadata:req.body.metadata,createdAt:now,updatedAt:now};await workflow.saveAppointment(appointment);lead.state=nextState;lead.updatedAt=now;await leadStore.save(lead);await leadEventStore.append({id:randomUUID(),leadId:lead.id,organizationId:c.tenantId,type:'lead.booked',actor:'sdr',timestamp:now,fromState:previousState,toState:nextState,reason:'appointment booked',metadata:{appointmentId:appointment.id,idempotencyKey}});if(webhookDispatcher)await webhookDispatcher.enqueue({id:randomUUID(),organizationId:c.tenantId,type:'appointment.booked',occurredAt:now,payload:{lead,appointment}});return res.status(201).json({appointment,lead});}catch(e){return res.status(tenantError(e)).json({error:e instanceof Error?e.message:'Unable to book appointment'});}});
...
async function start(){if(usePostgres){await runPostgresMigrations(db!);}app.listen(port,()=>console.log(`Zeerocodes Revenue Engine listening on ${port}`));}
